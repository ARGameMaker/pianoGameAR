...
public class GUIConrtol_note : MonoBehaviour {
	...
	// 必要なオブジェクトや、情報。.
	// 필요한 오브젝트 혹은 정보.
	public GameObject[] chrModel; // Characte prehab. 读取角色模型（作为GameObject）
	private NoteAnimatorControl chrCtrl; // Characte prehab. 读取角色模型（作为NoteAnimator），用作动画控制
	private int activeLodIdx = 0; // Index of active object in chrModel. 角色模型的活动索引，即将要控制的角色
	private int shaderIdx = 0; // Index of shader in use. 色号
	private int stateLength; // how much state is use. 

	// txt file used to property of UI element.
	public TextAsset stateList; // stateName文件
	public TextAsset IMInfo; // IMInfo文件
	private string[] stateName; // state name in animator 动画文字
	private string[] stateLabelName; // state name in UI UI文字
	public GameObject[] lightObj; // light object. 光线对象

	// GUI Object
	public GameObject AnimationSelectUI; // 选择动画按钮
	public GameObject InteractiveModeUI; // 选择模式按钮
	public GameObject ModelInformationUI;// 模式的说明信息
	public Text modelChangeButtonlabel; // 将要转换的模式文本
	private GameObject[] ASelectBtn = new GameObject[12]; // 按钮
	private Text[] ASelectLabel = new Text[12]; // 文本框
	private Text ASelectPage; // 页数
	private Text IMText; // 模式的说明文本
    ...
    // 动画放映的按钮实现（包括页码），这是在viewer mode中可被使用的
	...
	// will load from txt file. 读取stateName中的数据。
